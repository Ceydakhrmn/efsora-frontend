  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-background rounded-lg shadow-lg p-6 w-full max-w-xs">
        <h2 className="text-lg font-bold mb-2">MFA/2FA Doğrulama</h2>
        <p className="mb-4 text-muted-foreground text-sm">
          {mfaType === 'TOTP' ? 'Authenticator uygulamanızdaki 6 haneli kodu girin.' : 'E-posta veya yedek kodunuzu girin.'}
        </p>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <Button type="submit" disabled={loading || code.length < 4} className="flex-1">
              {loading ? <Loader2 className="h-4 w-4 animate-spin mx-auto" /> : 'Doğrula'}
            </Button>
          </div>
        </form>
      </div>
